...
echo "$last_immigrant_commit" | egrep '[a-f0-9]{40} [a-f0-9]{40} merged' || die "commit graph not imported (bad last commit)"
first_immigrant_commit=$(git log --format=oneline --parents immigrant | tail -1)
echo "$first_immigrant_commit" | egrep '[a-f0-9]{40} commit 1' || die "commit graph not imported (bad first commit)"
...
last_immigrant_commit=$(git log --format=oneline --parents -n 1 immigrant)
echo "$last_immigrant_commit" | egrep '[a-f0-9]{40} branch b' || die "commit graph not imported (bad last commit)"
