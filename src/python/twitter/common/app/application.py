...
class ApplicationThread(threading.Thread):
  def __init__(self, main_method):
    super(ApplicationThread, self).__init__(name='twitter.common.app Application Thread')
  ...
  def run(self):
    try:
      self.__return_code = self.__main_method()
    except SystemExit as e:
      self.__return_code = e.code
    except Exception as e:
      self.__return_code = 1
      _, _, self.__traceback = sys.exc_info()
    self.stop()
...
class Application(object):
  ...
  NO_COMMAND = 'DEFAULT'
  CTRL_C_RETURN_CODE = 130  # see http://tldp.org/LDP/abs/html/exitcodes.html
  ...
  def command_parser(self, command):
    ...
    for option in getattr(self._commands[command], self.OPTIONS_ATTR):
  ...
  def register_shutdown_command(self, command):
    ...
    if self._state not in (self.INITIALIZING, self.RUNNING):
