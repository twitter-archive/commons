...
# A CacheKey represents some version of a build entity (e.g., a target or set of targets).
#  - id identifies the entity.
#  - hash is a fingerprint of all invalidating inputs to the build step, i.e., it uniquely determines
#    a given version of the artifacts created when building the entity.
#  - num_sources is the number of source files used to build this version of the entity. Needed only
...
class CacheKeyGenerator(object):
  """Generates cache keys for versions of build entities."""
  ...
  def key_for_target(self, target, sources=TARGET_SOURCES, fingerprint_extra=None):
    return self.key_for_targets([target], sources, fingerprint_extra)

  def key_for_targets(self, targets, sources=TARGET_SOURCES, fingerprint_extra=None):
    """Get a key representing the given set of targets and their sources.

    :targets: The targets to create a CacheKey for.
    :sources: A source scope to select from the targets for hashing, defaults to TARGET_SOURCES.
    ...
    return CacheKey(Target.maybe_readable_identify(targets), sha.hexdigest(), num_sources)
...
# A persistent map from entity (e.g., target) id to cache key, which is a fingerprint of all
# the inputs to the current version of that entity. That cache key can then be used to look up
# build artifacts in an artifact cache.
class BuildInvalidator(object):
  ...
  def update(self, cache_key):
    """Makes cache_key the valid version of the corresponding entity.
