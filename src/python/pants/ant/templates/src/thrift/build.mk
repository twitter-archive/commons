...
<project name="${lib.id}" default="compile" basedir=".">
  ...
  <fileset id="${lib.id}.sources" dir="${"${"}thrift.src.dir}">
  % for path in lib.sources:
    <include name="${path}"/>
  % endfor
  </fileset>

  <fileset id="${lib.id}.thrift.sources" dir="${"${"}thrift.src.dir}">
    % for path in lib.sources:
    <include name="${path}"/>
    % endfor
  </fileset>
  ...
  <!-- override package-sources to include thrift idl as well as generated thrift java sources -->
  <target name="package-sources" depends="generate-thrift, compile.package-sources">
    <jar jarfile="${"${"}dist.dir}/${"${"}compile.jar.name}-sources.jar" update="true">
      <fileset refid="${lib.id}.thrift.sources"/>
      <fileset dir="${"${"}thrift.gen.dir}/gen-java"/>
    </jar>
  </target>
  ...
  <target name="_push-single">
    <fail>
      Only bare (non-meta) java_thrift_library modules with a provides key can have their artifacts
      pushed.
    </fail>
  </target>
